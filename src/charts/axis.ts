...
export class Axis {
  ...
  public static hasXAxis(chart: IChart) {
      let result = true;
  ...
  public static hasYAxis(chart: IChart) {
      let result = true;
      if (chart.axis !== undefined) {
  ...
  public static hasAxis(chart: IChart) {
      return this.hasXAxis(chart) || this.hasYAxis(chart);
  }

  public static generateXAxis(width: number, height: number, margin: number, chart: IChart, theme: ITheme) {
      let axis = "";
      ...
      for (let i = 0; i < tickCount; i++) {
          ...
          axis += "<text x=" + x +
              ...
              "style=\"font-family: sans-serif; font-size: 48px\">" +
              // "Jag heter Tom" +
              labels[i] +
              "</text>";
      }
  ...
  public static generateYAxis(width: number, height: number, margin: number, chart: IChart, theme: ITheme) {
      let axis = "";
      const spaceBetweenBars = 0;
      const tickLength = 25;
      const axisDist = margin; // Math.floor(margin / 2);
  ...
  public static getXAxisLabels(chart: IChart) {
      let labels: string[];
      if (chart.data.x) {
  ...
  public static addMissingXValues(data: string[]) {
      const numbers = data.map((x: string) => Number(x));
  ...
  public static calculateTicks(min: number, max: number, tickCount: number) {
      const span = max - min;
  ...
  public static getMaxValue(chart: IChart) {
      let max: number;
      let index = 0;
      if (chart.data.x) {
          const xlabel = chart.data.x;
  ...
  public static isXAxisTypeContinuous(chart: IChart) {
      let result = false;
      if (chart.axis) {
          if (chart.axis.x) {
