define(["require", "exports"], function (require, exports) {
    ...
    class Axis {
        constructor() { }
        static generateAxis(width, height, margin, chart, theme) {
            let axis = "";
        ...
        static hasAxis(chart) {
            return this.hasXAxis(chart) || this.hasYAxis(chart);
        }
        static generateXAxis(width, height, margin, chart, theme) {
            let axis = "";
        ...
        static generateYAxis(width, height, margin, chart, theme) {
            let axis = "";
            const spaceBetweenBars = 0;
            const tickLength = 25;
            const axisDist = margin; // Math.floor(margin / 2);
        ...
        static getXAxisLabels(chart) {
            let labels;
            if (chart.data.x) {
                const xlabel = chart.data.x;
                const index = chart.data.columns.findIndex((column) => column.name === xlabel);
                labels = chart.data.columns[index].values.map((x) => String(x));
                if (this.isXAxisTypeContinuous(chart)) {
                    labels = this.addMissingXValues(labels);
                }
            }
            else {
                labels = chart.data.columns[0].values.map((x, i) => String(i + 1));
            }
            return labels;
        }
        static addMissingXValues(data) {
        ...
        static calculateTicks(min, max, tickCount) {
            const span = max - min;
        ...
        static getMaxValue(chart) {
            let max;
            let index = 0;
            if (chart.data.x) {
                const xlabel = chart.data.x;
                index = chart.data.columns.findIndex((column) => column.name !== xlabel);
            }
        ...
        static isXAxisTypeContinuous(chart) {
            let result = false;
            if (chart.axis) {
                if (chart.axis.x) {
